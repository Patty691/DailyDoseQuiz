...
def vectorize_medicine_info(medicine_info: str) -> FAISS:
    """
    Vectoriseer de medicatie-informatie en sla deze op in een FAISS-database.
    """
    # Splits de tekst in secties
    sections = medicine_info.split("\n\n")
    
    # Filter secties met minder dan 20 woorden
    filtered_sections = [
        section.strip() for section in sections if len(section.split()) >= 20
    ]

    # Controleer of er secties zijn
    if not filtered_sections:
        raise ValueError("Geen secties met voldoende woorden gevonden in de medicatie-informatie.")

    # Maak embeddings aan
    embeddings = OpenAIEmbeddings()
    vectorstore = FAISS.from_texts(filtered_sections, embeddings)
...
# Functie om relevante informatie op te halen
def retrieve_relevant_info(vectorstore: FAISS, query: str) -> str:
    """
    Haal relevante informatie op uit de vector-database op basis van de query.
    """
    results = vectorstore.similarity_search(query, k=4 )  # Haal de top x relevante secties op
...
if __name__ == "__main__":
    ...
    # Vectoriseer de medicatie-informatie
    vectorstore = vectorize_medicine_info(medicine_info)
    ...
    query = f"Geef informatie over {random_category} van {medicine_name}."
    relevant_info = retrieve_relevant_info(vectorstore, query)
    print("DEBUG: Relevante informatie:")
    print(relevant_info)

    # Genereer een quizvraag
    try:
        print("Bezig met genereren van quizvraag...")
        response = generate_quiz_question(query, relevant_info, random_category)
